...
export function analyzeProduct(product, hasImage) {
  ...
  let report = `【TradePilot Global 出海选品前置决策报告】
...
说明：当前为基础毛利测算，按用户填写的建议售价与单件成本计算；国际物流、平台佣金、税费、包装和售后成本仍需按目标市场与实际渠道另行校正。
      ...
      <div className="min-h-screen bg-[#08100d] text-white">
        ...
        <main className="relative mx-auto flex min-h-screen max-w-7xl flex-col justify-center px-6 py-12">
          ...
          <h1 className="max-w-5xl text-5xl font-black leading-tight tracking-tight md:text-6xl xl:text-7xl">
            <span className="block text-white">TradePilot Global</span>
            <span className="mt-3 block text-emerald-300">出海前，先算清楚</span>
          </h1>

          <p className="mt-6 max-w-3xl text-base leading-8 text-slate-300 md:text-lg">
            面向义乌小商品卖家、小微跨境电商团队和新手跨境创业者的 AI 决策工作台。当前提交版聚焦出海选品前置判断：先识别商品、测算基础利润、提示风险，再完成内容测款、候选比较与复盘沉淀。
          </p>
          <div className="mt-5 max-w-4xl rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm leading-7 text-cyan-100">
            当前为 TradePilot Global 跨境基础版：无需注册即可体验完整闭环。国际物流、平台佣金、税费与合规专项模块将在后续版本增量接入；本版不伪造平台真实数据，也不自动下单或付款。
          </div>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            <CoverCard title="项目介绍" desc="了解 TradePilot Global 的目标用户、当前能力边界与迭代方向。" onClick={() => { setPage("app"); setMode("intro"); }} />
            <CoverCard title="开始出海选品判断" desc="上传图片并填写拿货价、MOQ，生成一份可复盘的出海选品前置报告。" highlight onClick={() => { setPage("app"); setMode("operate"); }} />
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-[#08100d]/85 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-4 md:flex-row md:items-center md:justify-between">
          <button onClick={() => setPage("cover")} className="text-left">
            <p className="text-sm text-emerald-300">TradePilot Global</p>
            <h1 className="text-xl font-black">出海选品前置判断、内容测款与产品复盘</h1>
          </button>
          ...
          <nav className="flex flex-wrap gap-2 text-sm font-bold">
            ...
            <Tab active={mode === "result"} onClick={() => setMode("result")}>选品报告</Tab>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-8">
        ...
        <h2 className="text-4xl font-black leading-tight">把“凭感觉选品”变成可记录、可比较、可复盘的出海前置决策流程。</h2>
        <p className="mt-5 text-base leading-8 text-slate-300">
          TradePilot Global 帮助小商品卖家在出海前完成第一轮结构化筛选：先看基础利润、MOQ、风险、人群和内容潜力，再把每次判断保存为候选产品记录，方便后续比较和复盘。
        </p>

        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          <Info title="核心痛点" items={painPoints} />
          <Info title="目标用户" items={["义乌小商品卖家", "小微跨境电商团队", "新手跨境创业者", "跨境新手卖家", "内容电商测试者", "需要选品复盘的团队"]} />
      ...
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <section className="rounded-[2rem] border border-white/10 bg-black/35 p-8">
          <p className="text-sm font-bold text-emerald-300">Why TradePilot Global</p>
          <h3 className="mt-2 text-2xl font-black text-white">为什么需要 TradePilot Global</h3>
          <p className="mt-5 text-sm leading-8 text-slate-300">
            很多小商品卖家和新手跨境创业者准备出海时，仍然依赖主观判断：觉得款式好看、拿货价便宜、供应商说好卖，就直接备货。但在真正投入前，基础利润是否够、MOQ 是否压货、商品是否值得优先拿样、内容能不能测出反馈，都需要先被结构化判断。
          </p>
          <p className="mt-4 text-sm leading-8 text-slate-300">
            TradePilot Global 先把出海前置选品过程结构化，让每次判断都留下数据、依据和复盘记录。国际物流、平台费用、税费与合规专项分析将在后续版本中继续增量接入。
          </p>
