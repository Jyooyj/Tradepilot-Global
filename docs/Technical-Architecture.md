# TradePilot Global Technical Architecture

本文面向开发者说明 TradePilot Global 的工程架构、Agent / Skills 协作、数据流、API、存储和 fallback 机制。README 面向使用者和评审快速体验；`docs/` 面向开发者理解实现边界。
...
TradePilot Global 当前采用一个工作流式 TradePilot Agent 编排多个 Skills，而不是多个独立 Agent 互相调用。Agent 根据当前商品、报告、产品库和复盘数据判断阶段，Skills 负责完成具体能力模块。
