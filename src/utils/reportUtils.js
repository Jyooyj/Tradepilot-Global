...
<head>
  <meta charset="utf-8">
  <title>TradePilot Global｜跨境候选产品库复盘报告</title>
  <style>
    body {
      margin: 0;
      padding: 28px;
...
<body>
  <h1>TradePilot Global｜跨境候选产品库复盘报告</h1>
  <div class="summary">
    <p><span class="strong">导出时间：</span>${escapeHtml(exportedAt)}</p>
    <p><span class="strong">产品记录数量：</span>${escapeHtml(records.length)}</p>
    <p><span class="strong">说明：</span>本报告由 TradePilot Global 根据本地产品库记录生成，用于出海候选产品复盘、产品对比和团队讨论。</p>
  </div>
  ...
  const linkCards = searchLinks.length
    ? searchLinks.map((link) => `
        <div class="card">
          ...
          <p><a href="${escapeHtml(link.url)}" target="_blank" rel="noreferrer">点击打开抖音搜索参考</a></p>
        </div>
      `).join("")
  ...
  const linkCards = searchLinks.length
    ? searchLinks.map((link) => `
        <div class="card">
          <h3>${escapeHtml(link.label)}</h3>
          <p>${escapeHtml(link.purpose)}</p>
          <p><a href="${escapeHtml(link.url)}" target="_blank" rel="noreferrer">点击打开淘宝搜索参考</a></p>
        </div>
      `).join("")
    : '<div class="card"><p>暂无搜索入口。</p></div>';
...
<head>
  ...
  <title>TradePilot Global｜出海选品前置决策报告</title>
  <style>
...
<body>
  <main class="page">
    <header class="hero">
      <h1>TradePilot Global｜出海选品前置决策报告</h1>
    ...
    <section class="notice">
      <h2>报告说明</h2>
      <p>本报告由 TradePilot Global 基于用户填写的产品信息、图片识别结果、成本结构、目标人群、销售渠道和测款逻辑自动生成。报告适用于进货前判断、内容测款准备、团队讨论和后续复盘，不构成绝对经营承诺。当前基础版未自动计算国际物流、平台佣金、税费或合规认证成本，实际备货前仍需人工核验。</p>
    </section>
  </main>
</body>
</html>`;
...
function injectPrintablePdfStyles(html) {
  ...
  const footer = `
    <footer class="pdf-report-footer">
      由 TradePilot Global 生成，仅供出海选品前置判断和测款参考。
    </footer>`;

  return String(html || "")
    .replace(/<title>.*?<\/title>/i, "<title>TradePilot Global｜出海选品前置决策报告</title>")
...
<body>
  <main class="page">
    ...
    ${renderDouyinEvidenceSection(douyinEvidence)}
    ${renderManualMarketEvidenceSection(manualMarketEvidence)}
    <footer class="footer">由 TradePilot Global 生成，仅供出海选品前置判断和测款参考。</footer>
  </main>
</body>
</html>`;
