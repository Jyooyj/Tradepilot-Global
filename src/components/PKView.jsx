...
export default function PKView({ records, loading, message, onRefresh, onRestore, leftId, setLeftId, rightId, setRightId }) {
  ...
  const sorted = [...safeRecords].sort((a, b) => (b.score || 0) - (a.score || 0));
    ...
    <section className="min-w-0 rounded-[2rem] border border-white/10 bg-white/[0.06] p-4 sm:p-6">
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="break-words text-2xl font-black sm:text-3xl">候选产品PK</h2>
          <p className="mt-2 break-words text-sm leading-7 text-slate-400">按已保存产品的评分排序，帮助你快速比较哪些款更适合优先拿样、继续测款或进入补货观察。</p>
        </div>
        <button onClick={onRefresh} className="min-h-11 w-full rounded-2xl bg-emerald-300 px-5 py-3 font-black text-black sm:w-auto">刷新候选池</button>
      </div>
      ...
      {sorted.length >= 2 && (
        <div className="space-y-5">
          ...
          <div className="overflow-x-auto rounded-3xl border border-white/10 bg-black/30">
            ...
            <div className="grid grid-cols-[minmax(120px,0.9fr)_minmax(160px,1fr)_minmax(160px,1fr)] border-b border-white/10 bg-white/[0.06] text-sm font-black text-slate-200">
      ...
      <div className="mt-6 grid gap-4">
        {sorted.map((record, index) => (
          <div key={record.id} className="min-w-0 rounded-3xl border border-white/10 bg-black/30 p-4 sm:p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="min-w-0">
                <p className="text-sm font-black text-emerald-300">#{index + 1}</p>
                <h3 className="break-words text-xl font-black sm:text-2xl">{getRecordMetrics(record).displayName}</h3>
                <p className="mt-2 break-words text-sm text-slate-400">{record.advice}</p>
              </div>
